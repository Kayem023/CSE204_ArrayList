...
void deleteItemAtSecond(int pos)
{
    if(pos>=length||pos<0){
        return;
    }
    if(length>1){
    for(int k=pos;k<length-2;k++){
        list[k]=list[k+1];
    }
    length--;
    }
...
{
    int first=searchItem(item);
    if(first==NULL_VALUE){
        return NULL_VALUE;
    }
    for(int i=first+1;i<length-1;i++){
        if(list[i]==item){
            deleteItemAtSecond(i);
        }
    }
}
